...
class Context:
    ...
    TCP_CONNECTIONS_FILE_PATH = ROUTER_ROOT_FOLDER + 'tcp_connections.csv'
    ...
    def __init__(self):
        with open(self.GROUND_TRUTH_MANIFEST_FILE, 'r') as f:
            self.trackables = json.load(f)["trackable_instances"]
            self.ip_to_trackable_mapping = {trackable["ip"]: trackable["tag"] for trackable in self.trackables}
            self.keys = set(self.ip_to_trackable_mapping.keys())
